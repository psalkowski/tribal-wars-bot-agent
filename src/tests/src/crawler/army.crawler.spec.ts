import { ArmyCrawler } from '../../../src/crawler/army.crawler.js';
...
describe('ArmyCrawler', () => {
  describe('crawl()', () => {
    test('should crawl data from train screen', async () => {
      const html = readFileSync(
        path.join(__dirname, '../../snapshots/rekrutacja.html'),
      ).toString();
    ...
    test('should crawl data from farm assistant', async () => {
      const html = readFileSync(
        path.join(__dirname, '../../snapshots/asystent-farmera.html'),
      ).toString();
      const crawler = new ArmyCrawler();
      const result = await crawler.crawl(html);
  ...
  describe('isSupported()', () => {
    ...
    test('should pass false when screen param is different than `train`', async () => {
      ...
      expect(
        await crawler.isSupported('http://example.com?screen=overview'),
      ).toBeFalsy();
    });
    ...
    test('should pass true when screen param is equal to `train`', async () => {
      ...
      expect(
        await crawler.isSupported('http://example.com?screen=train'),
      ).toBeTruthy();
    });
  });
});
