...
import { AttackCommand } from '../../../src/commands/attack.command.js';
import { ArmyUnit } from '../../../src/constants/army.js';
import { AttackType } from '../../../config/pl156.js';
...
describe('AttackCommand', () => {
  describe('constructor()', () => {
    test('should parse payload', () => {
      ...
      expect(command.arriveAt).toEqual(
        moment('29.08.2020 21:55:00.100', 'DD.MM.YYYY HH:mm:ss.SSS').valueOf(),
      );
      expect(command.runAt).toEqual(
        command.arriveAt - command.getDistanceDuration(),
      );
