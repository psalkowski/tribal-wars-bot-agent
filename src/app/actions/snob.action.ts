...
export class SnobAction extends Action {
  ...
  async handle(page: Page): Promise<Action> {
    ...
    await page.goto(
      'https://pl156.plemiona.pl/game.php?village=13134&screen=place&target=11135',
    );

    await page.waitForSelector('#command-data-form');
    await page.$eval(
      '#command-data-form input[name=axe]',
      (el: HTMLInputElement) => (el.value = '100'),
    );
    await page.$eval(
      '#command-data-form input[name=spy]',
      (el: HTMLInputElement) => (el.value = '1'),
    );
    await page.$eval(
  ...
  async isSupported(page: Page): Promise<boolean> {
    return (
      (await page.$(
        '#unit_overview_table .all_unit .unit_link[data-unit="snob"]',
      )) !== null
    );
  }
}
